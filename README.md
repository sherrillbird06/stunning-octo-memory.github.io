...
X               back
Enter           menu (map, journal, register, controls, evidence, export, status)
M               world map
F               fullscreen
C               toggle CRT scanlines
```

Sound is off until you press a key — browsers block audio before a gesture. The SOUND button in the top right toggles it.

On phones and tablets an on-screen D-pad and A/B buttons appear automatically.

### The story

Meridian Logistics failed its SOC 2 Type II ninety-one days ago. Eleven controls were attested as implemented; not one had dated evidence behind it. The analyst who signed that attestation resigned in March. You were hired last Tuesday, onto the helpdesk, at their old desk. Nobody cleaned out the drawers.

Every finding you log is a piece of what they left behind. The dormant admin account has their name on it. The ticket flood is a script they wrote to keep a metric green. The register you build is the thing they never had.

The **JOURNAL** in the menu keeps a running record of what you've worked out so far. The **MAP** (`M`) shows the building: the lift shaft is the region, each floor is an act, and the locked ones are the acts still to build.
...
- `MON` sprites are 16×16 arrays of `#`, `.`, `:`, ` `, `t` (transparent), drawn at 2× in battle. Keep every row exactly 16 characters. Assign each one a palette in `MONPAL`.
- Story lives in `PROLOGUE`, `BEATS`, `ENDING` and `JOURNAL`. Cutscenes paginate and word-wrap themselves, so write prose and let the engine break it.
- `FLOORS` drives the world map. When you build Act 2, bump `S.act` and its floor unlocks.
- `TRACKS` holds the music as arrays of MIDI numbers, 16 steps per bar. `SFX` holds the effects as functions of a start time.
...
The game is ~1,700 lines of vanilla JavaScript rendering to a 160×144 canvas — the actual Game Boy resolution — through a palette-indexed framebuffer, then scaled to fill the window. The 5×7 bitmap font is hand-built glyph by glyph rather than loaded, so it renders identically everywhere and never falls back to a system face.

Colour works the way the Game Boy Color did: all art is authored in four indices (0 darkest to 3 lightest), and each draw call selects one of 28 palette banks that map those indices to real colours. Adding a new creature type or a new room mood is one `bank()` line, not a repaint. The 5×7 bitmap font is hand-built glyph by glyph rather than loaded, so it renders identically everywhere and never falls back to a system face.

The canvas fills the viewport at integer scale where one fits, with a CSS scanline and vignette overlay you can switch off. Touch controls appear on coarse-pointer devices only.

Audio is synthesised at runtime with the Web Audio API — two pulse voices, a triangle bass and a noise channel, which is roughly the Game Boy's voice allocation. There are no audio files to load or host. Six looping tracks and sixteen effects live in about 120 lines of note arrays, so writing a new battle theme means typing MIDI numbers, not opening a DAW.
